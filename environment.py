...
class Env(tk.Tk):
    ...
    def _build_canvas(self):
        ...
        # 캔버스에 이미지 추가
        x, y = UNIT/2, UNIT/2
        self.boat = canvas.create_image(x, y, image=self.shapes[0])
    ...
    def load_images(self):
        ...
        boat = PhotoImage(
            Image.open(os.path.join(img_path, 'boat.png')).resize((30, 30)))
        fish = PhotoImage(
            Image.open(os.path.join(img_path, 'fish.png')).resize((30, 30)))
        port = PhotoImage(
            Image.open(os.path.join(img_path, 'port.png')).resize((30, 30)))
    ...
    def step(self, action):
        ...
        next_coords = self.move(self.boat, action)
        check = self.check_if_reward(self.coords_to_state(next_coords))
        done = check['if_goal']
        reward = check['rewards']

        self.canvas.tag_raise(self.boat)
    ...
    def move_const(self, target):
        ...
        if (target['figure'] is not self.boat
